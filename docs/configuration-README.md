...
* `VOICE_BRIDGE_MIN`: The minimum length (number of digits) of voice bridge numbers to allocate
* `VOICE_BRIDGE_MAX`: The maximum length (number of digits) of voice bridge numbers to allocate
* `DEFAULT_MEETING_TTL`: The length in seconds, before redis removes obsolete entries. Useful, if you have meetings running for multiple days.
