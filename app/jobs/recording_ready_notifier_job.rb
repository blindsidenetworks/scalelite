...
class RecordingReadyNotifierJob < ApplicationJob
  ...
  def perform(recording_id)
    ...
    notify(callback_url, meeting_id, recording_id) if callback_url
  end

  def encoded_payload(meeting_id, recording_id)
    secret = Rails.configuration.x.loadbalancer_secrets[0]
    payload = { meeting_id: meeting_id, recording_id: recording_id }
